#Embedded file name: toontown.safezone.PicnicGameTutorial
import inspect
...
class GameTutorial(DirectFrame, FSM.FSM):
    ...
    def __init__(self, doneFunction, doneEvent = None, callback = None):
        ...
        DirectFrame.__init__(self, pos=(-0.5, 0, 0), image_color=ToontownGlobals.GlobalDialogColor, image_scale=(1.0, 1.5, 1.0), text='', text_scale=0.06)
        self['image'] = DGG.getDefaultDialogGeom()
        self.title = DirectLabel(self, relief=None, text='', text_pos=(0.0, 0.35), text_fg=(1, 0, 0, 1), text_scale=0.13, text_font=ToontownGlobals.getSignFont())
        buttons = loader.loadModel('phase_3/models/gui/dialog_box_buttons_gui.bam')
        gui = loader.loadModel('phase_3.5/models/gui/friendslist_gui.bam')
        self.nextButton = DirectButton(self, image=(gui.find('**/Horiz_Arrow_UP'),
        ...
        self.prevButton = DirectButton(self, image=(gui.find('**/Horiz_Arrow_UP'),
         gui.find('**/Horiz_Arrow_DN'),
         gui.find('**/Horiz_Arrow_Rllvr'),
         gui.find('**/Horiz_Arrow_UP')), image3_color=Vec4(1, 1, 1, 0.5), image_scale=(-1.0, 1.0, 1.0), relief=None, text=TTLocalizer.ChineseTutorialPrev, text3_fg=Vec4(0, 0, 0, 0.5), text_scale=0.05, text_pos=(0.0, -0.1), pos=(-0.3, -0.3, -0.33), command=self.requestPrev)
        self.prevButton.hide()
        self.quitButton = DirectButton(self, image=(buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr')), relief=None, text=TTLocalizer.ChineseTutorialDone, text_scale=0.05, text_pos=(0.0, -0.1), pos=(0.35, -0.3, -0.33), command=self.handleQuit)
...
class CheckersTutorial(GameTutorial):
    ...
    def enterPage3(self, *args):
        ...
        self.blinker = Sequence(LerpColorInterval(self.obj, 0.5, Vec4(0.5, 0.5, 0, 0), Vec4(0.9, 0.9, 0, 1)), LerpColorInterval(self.obj, 0.5, Vec4(0.9, 0.9, 0, 1), Vec4(0.5, 0.5, 0, 0)))
        self.blinker.loop()
        self.page3.show()
...
class ChineseCheckersTutorial(GameTutorial):
    ...
    def enterPage1(self, *args):
        ...
        obj.show()
        self.blinker = Sequence(LerpColorInterval(obj, 0.5, Vec4(0.5, 0.5, 0, 0.0), Vec4(0.2, 0.2, 0.2, 1)), LerpColorInterval(obj, 0.5, Vec4(0.2, 0.2, 0.2, 1), Vec4(0.5, 0.5, 0, 0.0)))
        self.blinker.loop()
        self.page1.show()
