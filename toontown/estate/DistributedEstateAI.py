...
class DistributedEstateAI(DistributedObjectAI):
    ...
    def initPets(self, owner):
        ...
        def _queryAccount(dclass, fields):
            ...
            def _queryToon(dclass, fields):
                ...
                if not petId:
                    return
                self.air.sendActivate(petId, self.air.districtId, self.zoneId, dclass=self.air.dclassesByName['DistributedPetAI'])
                self.pets.append(petId)

            for avId in avatarList:
                if not avId:
                    continue
                self.air.dbInterface.queryObject(self.air.dbId, avId, callback=_queryToon)

        self.air.dbInterface.queryObject(self.air.dbId, owner.DISLid, callback=_queryAccount)
    ...
    def requestServerTime(self):
        self.sendUpdateToAvatarId(self.air.getAvatarIdFromSender(), 'setServerTime', [time.time() % HouseGlobals.DAY_NIGHT_PERIOD])
