#Embedded file name: toontown.estate.DistributedGardenPlotAI
...
class DistributedGardenPlotAI(DistributedLawnDecorAI):
    ...
    def plantStatuary(self, species):
        ...
        def _plant(task):
            if not self.air:
                return
            statuary = self.mgr.placeStatuary(self.mgr.S_pack(0, 0, species, 0), plot=self, ownerIndex=self.ownerIndex, plotId=self.plot, pos=(self.getPos(), self.getH()), generate=False)
