#Embedded file name: toontown.pandautils.BpDb
import inspect
import sys
...
class bp():
    ...
    def displayContextHint(self, displayPrefix = ''):
        contextString = displayPrefix + bpdb.bpMan.partsToPath({'id': self.id,
         'grp': self.grp,
         'cfg': self.cfg})
    ...
    def shouldBreak(self, test = None):
        ...
        if not bpdb.bpMan.getEnabled({'grp': self.grp,
         'id': self.id}):
            return False
        if not bpdb.verifyConfig(self.cfg):
            return False
        if bpdb.bpMan.getIgnoreCount({'grp': self.grp,
         'id': self.id}, decrement=True):
            return False
        if bpdb.bpMan.getLifetime({'grp': self.grp,
         'id': self.id}) == 0:
            return False
        return True

    def doBreak(self, frameCount = 1, displayPrefix = ''):
        bpdb.bpMan.decLifetime({'grp': self.grp,
         'id': self.id})
        bpdb.bpMan.incHitCount({'grp': self.grp,
         'id': self.id})
