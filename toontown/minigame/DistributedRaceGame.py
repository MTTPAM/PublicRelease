#Embedded file name: toontown.minigame.DistributedRaceGame
...
class DistributedRaceGame(DistributedMinigame):
    ...
    def __init__(self, cr):
        ...
        self.posHprArray = (((-9.03, 0.06, 0.025, -152.9),
    ...
    def runInPlace(self, avatar, lane, currentPlace, newPlace, time):
        ...
        runSeq = Sequence(Func(avatar.setAnimState, 'run', 1), avatar.posQuatInterval(time / 3.0, pos1[:3], pos1Quat, other=self.raceBoard), avatar.posQuatInterval(time / 3.0, pos2[:3], pos2Quat, other=self.raceBoard), avatar.posQuatInterval(time / 3.0, pos3[:3], pos3Quat, other=self.raceBoard), Func(stopRun))
        runSeq.start()
