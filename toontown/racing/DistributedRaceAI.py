#Embedded file name: toontown.racing.DistributedRaceAI
...
class DistributedRaceAI(DistributedObjectAI, FSM):
    ...
    def avatarFinished(self, avId):
        ...
        self.sendUpdate('setPlace', [avId,
         totalTime,
         place,
         entryFee,
         qualify,
         max(winnings - entryFee, 0),
         bonus,
         trophies,
         [],
         0])
    ...
    def requestThrow(self, x, y, z):
        ...
        if self.avatarGags[avId] == RaceGlobals.BANANA:
            ...
            for i in places:
                if i not in self.finishedAvatars and not self.currentlyAffectedByAnvil[i]:
                    currAvatar = i
                    break
