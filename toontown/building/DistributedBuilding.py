...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def enterWaitForVictors(self, ts):
        ...
        if victorCount == 1:
            ...
            base.camLens.setMinFov(settings['fieldofview'] / (4.0 / 3.0))
            anyOthers = 0
    ...
    def setupCogdo(self, nodePath):
        ...
        signTextNodePath = backgroundNP.attachNewNode(textNode.generate())
        signTextNodePath.setPosHprScale(0.0, -0.001, -0.13 + textHeight * 0.1 / zScale, 0.0, 0.0, 0.0, 0.8 / 20.0, 0.1, 0.1 / zScale)
        signTextNodePath.setColor(1.0, 1.0, 1.0, 1.0)
        if chr(self.track) == 'l':
    ...
    def walkOutCameraTrack(self):
        ...
        track = Sequence(Func(camera.reparentTo, render), Func(camera.setPosHpr, self.elevatorNodePath, 0, -32.5, 9.4, 0, 348, 0), Func(base.camLens.setMinFov, settings['fieldofview'] / (4.0 / 3.0)), Wait(VICTORY_RUN_TIME), Func(camera.setPosHpr, self.elevatorNodePath, 0, -32.5, 17, 0, 347, 0), Func(base.camLens.setMinFov, 75.0 / (4.0 / 3.0)), Wait(TO_TOON_BLDG_TIME), Func(base.camLens.setMinFov, settings['fieldofview'] / (4.0 / 3.0)), Func(camera.wrtReparentTo, base.localAvatar), camera.posQuatInterval(1.5, (0, -9 * scaleFactor, avHeight), (0, 0, 0), other=base.localAvatar, blendType='easeInOut'))
        return track
