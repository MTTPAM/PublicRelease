#Embedded file name: toontown.pickatoon.PickAToonOptions
...
class NewPickAToonOptions(DirectObject):
    ...
    def showOptions(self, animate = True):
        ...
        if animate:
            ...
            self.optionsNode.setPos(0, 0, -0.15)
            self.optionsNode.posInterval(0.15, Point3(0, 0, 0)).start()
            LerpColorScaleInterval(self.optionsNode, 0.15, Vec4(1, 1, 1, 1), Vec4(1, 1, 1, 0)).start()

    def hideOptions(self, animate = True):
        self.ignore('window-event')
        if animate:
            self.optionsNode.setColorScale(1, 1, 1, 1)
            self.optionsNode.posInterval(0.15, Point3(0, 0, -0.15)).start()
            LerpColorScaleInterval(self.optionsNode, 0.15, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1)).start()
            Sequence(Wait(0.15), Func(self.delAllOptions)).start()
        else:
            self.delAllOptions()
    ...
    def displayOptions(self):
        ...
        self.soundOptionsButton = DirectButton(relief=None, text_style=3, text_fg=(1, 1, 1, 1), text='Sound', text_scale=0.1, scale=0.95, command=self.displaySoundOptions)
        self.soundOptionsButton.reparentTo(self.optionsNode)
        self.soundOptionsButton.setPos(-0.6, 0, 0.7)
        self.soundOptionsButton.show()
        self.controlOptionsButton = DirectButton(relief=None, text_style=3, text_fg=(1, 1, 1, 1), text='Controls', text_scale=0.1, scale=0.95, command=self.displayControlOptions)
        ...
        self.videoOptionsButton = DirectButton(relief=None, text_style=3, text_fg=(1, 1, 1, 1), text='Video', text_scale=0.1, scale=0.95, command=self.displayVideoOptions)
    ...
    def displaySoundOptions(self):
        ...
        self.Music_toggleSlider = DirectSlider(parent=self.optionsNode, pos=(0, 0, 0.3), value=settings['musicVol'] * 100, pageSize=5, range=(0, 100), command=self.__doMusicLevel, thumb_geom=self.guiButton.find('**/QuitBtn_UP'), thumb_relief=None, thumb_geom_scale=1)
        self.Music_toggleSlider.setScale(0.4, 0.4, 0.4)
        self.Music_toggleSlider.show()
        self.SoundFX_toggleSlider = DirectSlider(parent=self.optionsNode, pos=(0, 0.0, 0.1), value=settings['sfxVol'] * 100, pageSize=5, range=(0, 100), command=self.__doSfxLevel, thumb_geom=self.guiButton.find('**/QuitBtn_UP'), thumb_relief=None, thumb_geom_scale=1)
    ...
    def displayVideoOptions(self):
        ...
        self.AspectRatioList = DirectOptionMenu(relief=None, parent=self.optionsNode, text_align=TextNode.ACenter, items=GraphicsOptions.AspectRatioLabels, command=self.__doWidescreen, text_scale=0.6, popupMarker_pos=(-1, 0, 0), popupMarker_relief=None, highlightScale=(1.1, 1.1), image=(self.guiButton.find('**/QuitBtn_UP'),
         self.guiButton.find('**/QuitBtn_DN'),
         self.guiButton.find('**/QuitBtn_RLVR'),
         self.guiButton.find('**/QuitBtn_UP')), image_scale=8, image3_color=Vec4(0.5, 0.5, 0.5, 0.5), text='', text3_fg=(0.5, 0.5, 0.5, 0.75), text_pos=(0, -0.02), pos=(0, 0, 0.3), image_pos=(0, 0, 0), item_text_align=TextNode.ACenter, popupMenu_text_scale=0.5, item_relief=None, item_pressEffect=1)
        ...
        self.fov_toggleSlider = DirectSlider(parent=self.optionsNode, pos=(0, 0, -0.1), value=settings['fieldofview'], pageSize=5, range=(30, 120), command=self.__doFovLevel, thumb_geom=self.guiButton.find('**/QuitBtn_UP'), thumb_relief=None, thumb_geom_scale=1)
        self.fov_toggleSlider.setScale(0.25)
        self.fov_resetButton = DirectButton(parent=self.optionsNode, relief=None, image=(self.guiButton.find('**/QuitBtn_UP'), self.guiButton.find('**/QuitBtn_DN'), self.guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text='Reset FOV', text_scale=0.052, text_pos=(0, -0.02), pos=(0, 0, -0.2), command=self.__resetFov)
        self.fovsliderText = OnscreenText('0.0', scale=0.3, pos=(0, 0.1), fg=(1, 1, 1, 1), style=3)
        self.fovsliderText.reparentTo(self.fov_toggleSlider.thumb)
        self.animations_Label = DirectLabel(parent=self.optionsNode, relief=None, text='', text_align=TextNode.ACenter, text_scale=0.052, text_wordwrap=16, pos=(0, 0, -0.3))
        self.animations_toggleButton = DirectButton(parent=self.optionsNode, relief=None, image=(self.guiButton.find('**/QuitBtn_UP'), self.guiButton.find('**/QuitBtn_DN'), self.guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text='', text_scale=0.052, text_pos=(0, -0.02), pos=(0, 0, -0.4), command=self.__doToggleAnimations)
        self.__doFovLevel()
