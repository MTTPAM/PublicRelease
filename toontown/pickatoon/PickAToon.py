#Embedded file name: toontown.pickatoon.PickAToon
...
BUTTONPOSITIONS = ((-1, 0, 0.5),
 ...
 (-0.6, 0, 0),
 (-0.2, 0, 0))
BUTTONPOSITIONSCLASSIC = (Vec3(-0.840167, 0, 0.359333),
...
class PickAToon(DirectObject):
    ...
    def __init__(self, avatarList, parentFSM, doneEvent):
        ...
        self.background2d.setColorScale(0.6, 0.1, 0.1, 0)
    ...
    def load(self):
        ...
        self.area = OnscreenText(parent=self.patNode2d, font=ToontownGlobals.getToonFont(), pos=(-0.1, -0.1), scale=0.075, text='', shadow=(0, 0, 0, 1), fg=COLORS[self.selectedToon])
        self.play = DirectButton(relief=None, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.73, 0.73), image2_scale=(0.83, 0.73, 0.73), text='PLAY THIS TOON', text_font=ToontownGlobals.getSignFont(), text_fg=(0.977, 0.816, 0.133, 1), text_pos=(0, -0.016), text_scale=0.035, scale=1.4, pos=(0, 0, -0.9), command=self.playGame, parent=self.patNode2d)
        self.toon = Toon.Toon()
        ...
        self.changeName = DirectButton(relief=None, image=(quitHover, quitHover, quitHover), text='NAME THIS TOON', text_font=ToontownGlobals.getSignFont(), text_fg=(0.977, 0.816, 0.133, 1), text_pos=(0, -0.016), text_scale=0.045, image_scale=1, image1_scale=1.05, image2_scale=1.05, scale=1.4, pos=(0, 0, -0.75), command=self.__handleNameYourToon, parent=self.patNode2d)
        LerpColorScaleInterval(self.patNode, 0.5, Vec4(1, 1, 1, 1), Vec4(1, 1, 1, 0)).start()
        LerpColorScaleInterval(self.patNode2d, 0.5, Vec4(1, 1, 1, 1), Vec4(1, 1, 1, 0)).start()
    ...
    def showToon(self):
        ...
        self.jumpIn = Sequence(Func(self.toon.loop, 'wave'), Wait(self.toon.getDuration('wave')), Func(self.toon.animFSM.request, 'neutral'))
        self.jumpIn.start()
    ...
    def playGame(self):
        if self.jumpIn:
            self.jumpIn.finish()
        doneStatus = {'mode': 'chose',
         'choice': self.selectedToon}
        messenger.send(self.doneEvent, [doneStatus])

    def makeToon(self, position = None):
        doneStatus = {'mode': 'create',
         'choice': self.selectedToon}
        messenger.send(self.doneEvent, [doneStatus])
    ...
    def setupButtons(self, av = None, position = 0):
        ...
        if av:
            ...
            deleteButton = DirectButton(parent=button, geom=(trashcanGui.find('**/TrashCan_CLSD'), trashcanGui.find('**/TrashCan_OPEN'), trashcanGui.find('**/TrashCan_RLVR')), text=('',
             TTLocalizer.AvatarChoiceDelete,
             TTLocalizer.AvatarChoiceDelete,
             ''), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.15, text_pos=(0, -0.1), relief=None, scale=0.5, command=self.__handleDelete, extraArgs=[position], pos=(0.2, 0, -0.2))
        else:
    ...
    def __handleDelete(self, position):
        ...
        def diagDone():
            ...
            if mode == 'ok':
                ...
                deleteText = TTLocalizer.AvatarChoiceDeleteConfirmText % {'name': av.name,
        ...
        delDialog = TTGlobalDialog(message=DEL % av.name, style=YesNo, doneEvent='pat-del-diag-done')
        base.transitions.fadeScreen(0.5)

    def __handleNameYourToon(self):
        doneStatus = {'mode': 'nameIt',
         'choice': self.selectedToon}
        messenger.send(self.doneEvent, [doneStatus])
    ...
    def openOptions(self):
        ...
        self.optionsButton.setPos(0, 1, -0.75)
    ...
    def quitGame(self):
        ...
        self.shardsButton.hide()
        self.quitButton.hide()
        LerpColorScaleInterval(self.patNode, 0.5, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1)).start()
        LerpColorScaleInterval(self.patNode2d, 0.5, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1)).start()

    def showQuitConfirmation(self):
        LerpColorScaleInterval(self.background2d, 0.5, Vec4(0.6, 0.1, 0.1, 0.5), startColorScale=Vec4(0.6, 0.1, 0.1, 0)).start()
    ...
    def doCancelQuitFunc(self):
        LerpColorScaleInterval(self.background2d, 0.5, Vec4(0.6, 0.1, 0.1, 0), startColorScale=Vec4(0.6, 0.1, 0.1, 0.5)).start()
