...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def getDialogueArray(self, *args):
        if hasattr(self, 'animalSound'):
            dialogueArrays = [Toon.DogDialogueArray,
    ...
    def setTalk(self, fromAV, fromAC, avatarName, chat, mods, flags):
        ...
        if fromAV == 0:
            self.notify.debug(':%s: setTalk: %r, %r, %r' % (timestamp,
             self.doId,
             self.name,
             chat))
        else:
    ...
    def setTalkWhisper(self, fromAV, fromAC, avatarName, chat, mods, flags):
        ...
        timestamp = time.strftime('%m-%d-%Y %H:%M:%S', time.localtime())
        print ':%s: receiveWhisperTalk: %r, %r, %r, %r, %r, %r, %r' % (timestamp,
         fromAV,
         avatarName,
         fromAC,
         None,
    ...
    def whisperSCToontaskTo(self, taskId, toNpcId, toonProgress, msgIndex, sendToId):
        messenger.send('wakeup')
        base.cr.ttaFriendsManager.d_whisperSCToontaskTo(sendToId, taskId, toNpcId, toonProgress, msgIndex)
        ...
        for id, timestamp in enumerate(self.buffs):
            if id == ToontownGlobals.BMovementSpeed:
                ...
                if ZoneUtil.getWhereName(self.zoneId, True) not in ('playground', 'street', 'toonInterior', 'cogHQExterior', 'factoryExterior'):
                    return
                self.controlManager.setSpeeds(ToontownGlobals.ToonForwardSpeed * ToontownGlobals.BMovementSpeedMultiplier, ToontownGlobals.ToonJumpForce, ToontownGlobals.ToonReverseSpeed * ToontownGlobals.BMovementSpeedMultiplier, ToontownGlobals.ToonRotateSpeed * ToontownGlobals.BMovementSpeedMultiplier)
    ...
    def magicTeleportInitiate(self, hoodId, zoneId):
        ...
        if ZoneUtil.isDynamicZone(zoneId) and hoodId in [ToontownGlobals.BossbotHQ,
         ToontownGlobals.LawbotHQ,
         ToontownGlobals.CashbotHQ,
         ToontownGlobals.SellbotHQ]:
            whereId = 'cogHQBossBattle'
        if zoneId in [ToontownGlobals.BossbotLobby,
