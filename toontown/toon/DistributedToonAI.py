...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    DbCheckPeriodPaid = simbase.config.GetInt('toon-db-check-period-paid', 600)
    DbCheckPeriodUnpaid = simbase.config.GetInt('toon-db-check-period-unpaid', 60)
    ...
    def setDNAString(self, string):
        ...
        if not self.verifyDNA():
            ...
            self.air.writeServerEvent('suspicious', self.doId, 'Invalid DNA string.')
    ...
    def doCogInvasion(self, suitIndex):
        ...
        if self.air.suitInvasionManager.startInvasion(suitDeptIndex=track, suitTypeIndex=type):
            return ['success', suitIndex, 0]
    ...
    def doDeptInvasion(self, deptIndex):
        ...
        if self.air.suitInvasionManager.startInvasion(suitDeptIndex=deptIndex, suitTypeIndex=None):
    ...
    def setAchievements(self, achievements):
        for i in xrange(len(achievements)):
            if achievements[i] not in xrange(len(Achievements.AchievementsDict)):
...
def cheesyEffect(value, hood = 0, expire = 0):
    ...
    if value not in invoker.cheesyEffects:
...
def maxToon(missingTrack=None):
    ...
    return "Succesfully maxxed your Toon. I dont see why you cant just max your toon yourself."
...
def warn(banWorthy):
    ...
    return '%(name)s now has %(warnings)d warnings.' % {'name': target.getName(),
     'warnings': target.getWarningCount()}
...
def inventory(a, b = None, c = None):
    ...
    if a == 'reset':
        ...
        if not -1 <= targetTrack < len(ToontownBattleGlobals.Tracks):
            return 'Invalid target track index: ' + str(targetTrack)
        elif targetTrack != -1 and not invoker.hasTrackAccess(targetTrack):
...
def dna(part, value):
    ...
    if part in ('head', 'species'):
        speciesNames = ('dog', 'cat', 'horse', 'mouse', 'rabbit', 'duck', 'monkey', 'bear', 'pig', 'deer')
        ...
        if dna.headColor == 26 and value == 'c':
            return 'Invalid species for color: black'
        if dna.headColor == 0 and value == 'b':
...
def track(command, track, value = None):
    try:
        index = ('toonup', 'trap', 'lure', 'sound', 'throw', 'squirt', 'zap', 'drop').index(track.lower())
    except:
...
def disguise(command, suitIndex, value):
    ...
    elif value < 0 and command != 'reviveLevel':
