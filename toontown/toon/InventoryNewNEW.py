#Embedded file name: toontown.toon.InventoryNewNEW
...
class InventoryNewNEW(InventoryBase.InventoryBase, DirectFrame):
    ...
    def load(self):
        ...
        for track in xrange(len(Tracks)):
            ...
            self.trackBars.append(DirectWaitBar(parent=self.trackRows[track], pos=(-0.58 + adjustLeft, -0.1, -0.025), relief=DGG.SUNKEN, frameSize=(-0.6, 0.6, -0.1, 0.1), borderWidth=(0.02, 0.02), scale=0.25, frameColor=(TrackColors[track][0] * 0.6,
             TrackColors[track][1] * 0.6,
             TrackColors[track][2] * 0.6,
        ...
        for x in xrange(8):
            self.accept('alt-%d' % (x + 1), self.doTab, extraArgs=[x])
    ...
    def doTab(self, index):
        ...
        for x in xrange(7):
            self.accept('control-%d' % (x + 1), self.__handleSelection, extraArgs=[self.activeTab, x, True])
    ...
    def showDetail(self, track, level, event = None):
        ...
        if self.propAndOrganicBonusStack:
            ...
            if damageBonus:
                damageBonusStr = TTLocalizer.InventoryDamageBonus % damageBonus
        accString = AvPropAccuracy[track][level]
        if track == SQUIRT_TRACK or track == TRAP_TRACK and organicBonus or track == DROP_TRACK and organicBonus or track == ZAP_TRACK:
            self.detailDataLabel.configure(text=TTLocalizer.InventoryDetailDataExtra % {'accuracy': str(accString) + '%',
            ...
            self.detailDataLabel.configure(text=TTLocalizer.InventoryDetailData % {'accuracy': str(accString) + '%',
             'damageString': self.getToonupDmgStr(track, level),
             'damage': damage,
             'bonus': damageBonusStr,
             'singleOrGroup': self.getSingleGroupStr(track, level)})
    ...
    def bookDeleteActivateButtons(self):
        ...
        self.setScale(0.8)
        self.deleteEnterButton.hide()
        self.deleteEnterButton.setPos(1.0, 0, -0.639)
        self.deleteEnterButton.setScale(0.75)
        self.deleteExitButton.show()
        self.deleteExitButton.setPos(1.0, 0, -0.639)
