#Embedded file name: toontown.shtiker.OptionsPageGUI
...
class OptionButton(DirectButton):
    ...
    def __init__(self, parent = None, wantLabel = False, z = buttonbase_ycoord, labelZ = None, labelOrientation = 'left', labelPos = None, labelText = '', image_scale = (0.7, 1, 1), text = '', **kw):
        loadModels()
        if parent is None:
            parent = aspect2d
        ...
        if wantLabel:
            self.label = OptionLabel(parent=self, z=labelZ, pos=labelPos, orientation=labelOrientation, text=labelText)
...
class OptionLabel(DirectLabel):
    ...
    def __init__(self, parent = None, z = textStartHeight, text_wordwrap = 16, text = '', orientation = 'left', **kw):
        loadModels()
        if parent is None:
            parent = aspect2d
        if orientation == 'left':
