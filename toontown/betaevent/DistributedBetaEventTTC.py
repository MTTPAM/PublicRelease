#Embedded file name: toontown.betaevent.DistributedBetaEventTTC
...
class DistributedBetaEventTTC(DistributedEvent):
    ...
    def __init__(self, cr):
        ...
        self.invasion1 = loader.loadMusic('phase_14/audio/bgm/event_temp_1.ogg')
    ...
    def enterEvent(self, timestamp):
        Sequence(Func(self.toonTalk, 'Helloooooooo Toontown!', self.prepostera), Wait(4), Func(self.toonTalk, 'We gathered you all here today...', self.prepostera), Wait(4), Func(self.toonTalk, 'To announce the reconstruction and opening of...', self.prepostera), Wait(5), Func(self.toonTalk, 'Loony La...', self.prepostera), Wait(3), Func(self.toonTalk, 'It seems like Doctor Dimm is trying to reach me...', self.prepostera)).start()

    def exitEvent(self):
        pass

    def enterHoncho(self, timestamp):
        Sequence(self.headHoncho1.beginSupaFlyMove(Vec3(68, -2, 4.024), True, 'firstCogInvadeFlyIn', walkAfterLanding=False), Func(self.headHoncho1.loop, 'walk'), self.headHoncho1.hprInterval(2, VBase3(90, 0, 0)), Func(self.headHoncho1.loop, 'neutral')).start()
