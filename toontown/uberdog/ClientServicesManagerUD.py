    ...
    def lookup(self, username, callback):
        if str(username) not in self.dbm:
            response = {'success': True,
             'userId': username,
             'accountId': 0,
             'accessLevel': max(400, minAccessLevel)}
            callback(response)
            return response
        else:
            response = {'success': True,
...
class LoginAccountFSM(OperationFSM):
    ...
    def enterSetAccount(self):
        if self.accessLevel:
            self.csm.air.dbInterface.updateObject(self.csm.air.dbId, self.accountId, self.csm.air.dclassesByName['AccountUD'], {'ACCESS_LEVEL': self.accessLevel})
        datagram = PyDatagram()
        datagram.addServerHeader(self.csm.GetAccountConnectionChannel(self.accountId), self.csm.air.ourChannel, CLIENTAGENT_EJECT)
        ...
        if access >= 500:
            ...
            self.csm.air.send(dg)
        datagram = PyDatagram()
        datagram.addServerHeader(self.target, self.csm.air.ourChannel, CLIENTAGENT_SET_CLIENT_ID)
    ...
    def enterSendAvatars(self):
        ...
        for avId, fields in self.avatarFields.items():
            ...
            if wishNameState == 'OPEN':
                ...
                self.csm.air.dbInterface.updateObject(self.csm.air.dbId, avId, self.csm.air.dclassesByName['DistributedToonUD'], {'WishNameState': [actualNameState]})
    ...
    def enterJudgeName(self):
        ...
        if self.avId and status:
            ...
            if resp != 'Success':
                ...
                self.csm.air.dbInterface.updateObject(self.csm.air.dbId, self.avId, self.csm.air.dclassesByName['DistributedToonUD'], {'WishNameState': ('PENDING',),
                 'WishName': (self.name,)})
        if self.avId:
...
class SetNamePatternFSM(AvatarOperationFSM):
    ...
    def enterSetName(self):
        ...
        name = ' '.join(parts)
        self.csm.air.dbInterface.updateObject(self.csm.air.dbId, self.avId, self.csm.air.dclassesByName['DistributedToonUD'], {'WishNameState': ('',),
         'WishName': ('',),
         'setName': (name,)})
...
class AcknowledgeNameFSM(AvatarOperationFSM):
    ...
    def __handleAvatar(self, dclass, fields):
        ...
        if wishNameState == 'APPROVED':
            ...
            return
        self.csm.air.dbInterface.updateObject(self.csm.air.dbId, self.avId, self.csm.air.dclassesByName['DistributedToonUD'], {'WishNameState': (wishNameState,),
         'WishName': (wishName,),
...
class LoadAvatarFSM(AvatarOperationFSM):
    ...
    def enterSetAvatar(self):
        ...
        self.csm.air.send(datagram)
        self.csm.air.sendActivate(self.avId, 0, 0, self.csm.air.dclassesByName['DistributedToonUD'], {'setAdminAccess': [self.account.get('ACCESS_LEVEL', 100)]})
        datagram = PyDatagram()
        ...
        datagram.addChannel(self.target << 32 | self.avId)
        self.csm.air.send(datagram)
        taskMgr.doMethodLater(0.2, self.enterSetAvatarTask, 'avatarTask-%s' % self.avId, extraArgs=[channel], appendTask=True)
