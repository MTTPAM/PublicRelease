#Embedded file name: toontown.dmenu.DMenuQuit
...
class DMenuQuit(DirectObject):
    ...
    def showConf(self):
        ...
        zoomIn = LerpScaleInterval(self.confNode, 0.4, Vec3(1, 1, 1), Vec3(0, 0, 0), blendType='easeInOut').start()

    def hideConf(self):
        base.transitions.noFade()
        zoomOut = LerpScaleInterval(self.confNode, 0.4, Vec3(0, 0, 0), Vec3(1, 1, 1), blendType='easeInOut').start()
    ...
    def displayConfirmation(self):
        ...
        self.yesBtn = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.lYes, text_pos=(0, -0.02), text_scale=0.07, scale=1.2, command=self.quitGame)
        self.yesBtn.reparentTo(self.confNode)
        self.yesBtn.setPos(-0.3, 0, -0.2)
        self.yesBtn.show()
        self.noBtn = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.lNo, text_pos=(0, -0.02), text_scale=0.07, scale=1.2, command=self.cancelQuitGame)
        self.noBtn.reparentTo(self.confNode)
        self.noBtn.setPos(0.3, 0, -0.2)
        self.noBtn.show()
