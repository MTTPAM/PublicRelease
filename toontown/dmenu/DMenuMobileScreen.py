#Embedded file name: toontown.dmenu.DMenuMobileScreen
...
class DMenuMobileScreen(DirectObject):
    ...
    def createButtons(self):
        ...
        self.PlayButton = DirectButton(relief=None, text_style=3, image=mPlay, image_scale=0.35, image1_scale=0.34, image2_scale=0.36, text_fg=(1, 1, 1, 1), text_pos=(0, -0.02), text_scale=0.07, scale=1.2, command=self.playGame)
        self.PlayButton.reparentTo(aspect2d)
        self.PlayButton.setPos(MPlayBtnPos)
        self.PlayButton.setTransparency(1)
        self.PlayButton.show()
        self.OptionsButton = DirectButton(relief=None, text_style=3, image=mOptions, image_scale=0.3, image1_scale=0.29, image2_scale=0.31, text_fg=(1, 1, 1, 1), text_pos=(0, -0.02), text_scale=0.08, scale=0.95, command=self.openOptions)
        ...
        self.QuitButton = DirectButton(relief=None, text_style=3, image=mQuit, image_scale=0.3, image1_scale=0.29, image2_scale=0.31, text_fg=(1, 1, 1, 1), text_pos=(0, -0.02), text_scale=0.08, scale=0.95, command=self.quitGame)
        self.QuitButton.reparentTo(base.a2dBottomRight)
        self.QuitButton.setPos(MQuitBtnPos)
        self.QuitButton.setTransparency(1)
        self.QuitButton.show()
        self.CreditsButton = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text='Credits', text_pos=(0, -0.02), text_scale=0.07, scale=0.95, command=self.startCredits)
    ...
    def playGame(self):
        ...
        if self.fadeOut is not None:
            self.fadeOut.finish()
            self.fadeOut = None
        Sequence(Func(self.doPlayButton), LerpColorScaleInterval(self.background2d, 0.5, Vec4(1, 1, 1, 0), startColorScale=Vec4(1, 1, 1, 1)), Func(self.enterGame)).start()
    ...
    def doPlayButton(self):
        ...
        Parallel(self.PlayButton.posInterval(0.2, Point3(MPlayBtnHidePos), blendType='easeInOut'), self.OptionsButton.posInterval(0.2, Point3(MOptionsBtnHidePos), blendType='easeInOut'), self.QuitButton.posInterval(0.2, Point3(MQuitBtnHidePos), blendType='easeInOut'), self.NewsButton.posInterval(0.2, Point3(MDiscordBtnHidePos), blendType='easeInOut'), self.CreditsButton.posInterval(0.2, Point3(MCreditsBtnHidePos), blendType='easeInOut'), self.logo.posInterval(0.5, Point3(0, 0, 2.5), blendType='easeInOut')).start()
    ...
    def showQuitConfirmation(self):
        LerpColorScaleInterval(self.background2d, 0.5, Vec4(0.6, 0.1, 0.1, 1), startColorScale=Vec4(1, 1, 1, 1)).start()
    ...
    def buttonInAnimation(self):
        ...
        play = self.PlayButton.posInterval(0.5, Point3(MPlayBtnPos), blendType='easeInOut')
        opt = self.OptionsButton.posInterval(0.5, Point3(MOptionsBtnPos), blendType='easeInOut')
        quit = self.QuitButton.posInterval(0.5, Point3(MQuitBtnPos), blendType='easeInOut')
        discord = self.NewsButton.posInterval(0.5, Point3(MDiscordBtnPos), blendType='easeInOut')
        credits = self.CreditsButton.posInterval(0.5, Point3(MCreditsBtnPos), blendType='easeInOut')
