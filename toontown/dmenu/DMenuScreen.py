#Embedded file name: toontown.dmenu.DMenuScreen
...
from panda3d.core import TransparencyAttrib, Point3, VBase3, Vec4, Vec3, TextNode
import random
import webbrowser
...
class DMenuScreen(DirectObject):
    ...
    def __init__(self):
        ...
        self.seq = None
        self.isBananaPlaying = False
        base.cr.avChoice = None
        self.allButtons = []
        base.transitions.fadeOut(0)
        fadeSequence = Sequence(base.transitions.getFadeInIval(1), base.camera.posHprInterval(1, Point3(MAIN_POS), VBase3(MAIN_HPR), blendType='easeInOut')).start()
        ...
        self.background2d.setColorScale(1, 1, 1, 0.6)
        ...
        self.logo = OnscreenImage(image=GameLogo, scale=(0.5, 0.5, 0.25))
    ...
    def enterReleaseNotes(self):
        if not self.releaseNotesBox:
            ...
            self.releaseNotesText = OnscreenText(text='Fetching Release Notes...', align=TextNode.ALeft, scale=0.05, wordwrap=50)
            self.releaseNotesText.reparent_to(base.a2dTopLeft)
            self.releaseNotesText.set_pos(0.4, 0, -0.4)
        ...
        if base.mouseWatcherNode.hasMouse():
            ...
            if self.collHandlerQueue.getNumEntries() > 0:
                ...
                if pickedObj == self.banana:
                    self.seq = Sequence(Func(setPlayingStatus, True), Func(self.surlee.play, surleeAnim), Func(self.dimm.play, dimmAnim), Wait(3), Func(self.surlee.loop, 'scientistGame'), Func(self.dimm.loop, 'scientistGame'), Func(setPlayingStatus, False))
    ...
    def createButtons(self):
        ...
        self.PlayButton = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.PlayGame, text_pos=(0, -0.02), text_scale=0.07, scale=1.2, command=self.playGame)
        self.PlayButton.reparentTo(aspect2d)
        self.PlayButton.setPos(PlayBtnHidePos)
        self.PlayButton.show()
        self.OptionsButton = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.OptionsPageTitle, text_pos=(0, -0.02), text_scale=0.08, scale=0.95, command=self.openOptions)
        self.OptionsButton.reparentTo(aspect2d)
        self.OptionsButton.setPos(OptionsBtnHidePos)
        self.OptionsButton.show()
        self.QuitButton = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.lQuit, text_pos=(0, -0.02), text_scale=0.08, scale=0.95, command=self.quitGame)
        self.QuitButton.reparentTo(aspect2d)
        self.QuitButton.setPos(QuitBtnHidePos)
        self.QuitButton.show()
        self.CreditsButton = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.CreditsButton, text_pos=(0, -0.02), text_scale=0.07, scale=0.95, command=self.startCredits)
        self.CreditsButton.reparentTo(aspect2d)
        self.CreditsButton.setPos(CreditsBtnHidePos)
        self.CreditsButton.show()
        self.NewsButton = DirectButton(relief=None, text_style=3, image=(shuffleUp, shuffleDown, shuffleUp), image_scale=(0.8, 0.7, 0.7), image1_scale=(0.83, 0.7, 0.7), image2_scale=(0.83, 0.7, 0.7), text_fg=(1, 1, 1, 1), text=TTLocalizer.DiscordButton, text_pos=(0, -0.02), text_scale=0.07, scale=0.95, command=self.enterReleaseNotes)
    ...
    def openOptions(self):
        ...
        self.closeOptionsButton.setPos(0, 1, -0.75)
        self.closeOptionsButton.show()
        for button in self.allButtons:
            LerpColorScaleInterval(button, 0.15, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1)).start()

    def hideOptions(self):
        self.optionsMgr.hideOptions()
        self.closeOptionsButton.hide()
        for button in self.allButtons:
            LerpColorScaleInterval(button, 0.15, Vec4(1, 1, 1, 1), Vec4(1, 1, 1, 0)).start()
    ...
    def playGame(self):
        ...
        Sequence(Func(self.doPlayButton), LerpColorScaleInterval(self.background2d, 0.5, Vec4(1, 1, 1, 0), startColorScale=Vec4(1, 1, 1, 0.6)), Func(self.enterGame), base.camera.posHprInterval(1, Point3(-36, -2, 12), VBase3(-90, -2, 0), blendType='easeInOut')).start()
    ...
    def doPlayButton(self):
        ...
        Parallel(self.PlayButton.posInterval(0.2, Point3(PlayBtnHidePos), blendType='easeInOut'), self.OptionsButton.posInterval(0.2, Point3(OptionsBtnHidePos), blendType='easeInOut'), self.QuitButton.posInterval(0.2, Point3(QuitBtnHidePos), blendType='easeInOut'), self.NewsButton.posInterval(0.2, Point3(DiscordBtnHidePos), blendType='easeInOut'), self.CreditsButton.posInterval(0.2, Point3(CreditsBtnHidePos), blendType='easeInOut'), self.logo.posInterval(0.5, Point3(0, 0, 2.5), blendType='easeInOut')).start()
    ...
    def showQuitConfirmation(self):
        LerpColorScaleInterval(self.background2d, 0.5, Vec4(0.6, 0.1, 0.1, 0.6), startColorScale=Vec4(1, 1, 1, 0.6)).start()
    ...
    def doCancelQuitFunc(self):
        LerpColorScaleInterval(self.background2d, 0.5, Vec4(1, 1, 1, 0.6), startColorScale=Vec4(0.6, 0.1, 0.1, 0.6)).start()
        self.buttonInAnimation()
        self.quitConfirmation.hideConf()

    def buttonInAnimation(self):
        logo = self.logo.posInterval(0.5, Point3(0, 0, 0.5), blendType='easeInOut')
        play = self.PlayButton.posInterval(0.2, Point3(PlayBtnPos), blendType='easeInOut')
        opt = self.OptionsButton.posInterval(0.2, Point3(OptionsBtnPos), blendType='easeInOut')
        quit = self.QuitButton.posInterval(0.2, Point3(QuitBtnPos), blendType='easeInOut')
        discord = self.NewsButton.posInterval(0.2, Point3(DiscordBtnPos), blendType='easeInOut')
        credits = self.CreditsButton.posInterval(0.2, Point3(CreditsBtnPos), blendType='easeInOut')
        Sequence(Func(logo.start), Wait(0.1), Func(play.start), Wait(0.1), Func(opt.start), Func(discord.start), Func(credits.start), Func(quit.start)).start()
