#Embedded file name: toontown.friends.TTAFriendsManagerUD
import string
import random
import functools
import time
...
class FriendsListOperation(OperationFSM):
    ...
    def enterRetrieved(self, friendsList):
        ...
        self.air.dbInterface.queryObject(self.air.dbId, self.friendsList[0][0], self.addFriend)
    ...
    def addFriend(self, dclass, fields):
        ...
        self.realFriendsList.append([friendId,
         fields['setName'][0],
        ...
        if len(self.realFriendsList) >= len(self.friendsList):
            ...
            return
        self.friendIndex += 1
        self.air.dbInterface.queryObject(self.air.dbId, self.friendsList[self.friendIndex][0], self.addFriend)
...
class RemoveFriendOperation(OperationFSM):
    ...
    def enterRetrieved(self, friendsList, trueFriendsList):
        ...
        if self.sender in self.mgr.onlineToons:
            dg = self.air.dclassesByName['DistributedToonUD'].aiFormatUpdate('setFriendsList', self.sender, self.sender, self.air.ourChannel, [newList])
            self.air.send(dg)
            if self.alert:
                dg = self.air.dclassesByName['DistributedToonUD'].aiFormatUpdate('friendsNotify', self.sender, self.sender, self.air.ourChannel, [self.target, 1])
                self.air.send(dg)
                self.demand('Off')
                return
        self.air.dbInterface.updateObject(self.air.dbId, self.sender, self.air.dclassesByName['DistributedToonUD'], {'setFriendsList': [newList],
         'setTrueFriends': [newTFList]})
        self.demand('Off')
...
class FriendDetailsOperation(OperationFSM):
    ...
    def enterRetrieved(self, friendsList):
        ...
        for id in self.friendIds:
            for friend in friendsList:
                if friend[0] == id:
                    self.currId = id
                    self.air.dbInterface.queryObject(self.air.dbId, id, self.handleFriend)
                    break
    ...
    def handleFriend(self, dclass, fields):
        ...
        self.mgr.sendUpdateToAvatarId(self.sender, 'friendInfo', [self.currId,
         name,
         dna,
         petId])
...
class ClearListOperation(OperationFSM):
    ...
    def enterRetrieved(self, friendsList):
        for friend in friendsList:
            newOperation = RemoveFriendOperation(self.mgr, self.air, friend[0], targetAvId=self.sender, alert=True)
...
class TTAFriendsManagerUD(DistributedObjectGlobalUD):
    ...
    def requestAvatarInfo(self, friendIdList):
        avId = self.air.getAvatarIdFromSender()
        newOperation = FriendDetailsOperation(self, self.air, avId, friendIds=friendIdList)
        self.operations.append(newOperation)
        newOperation.demand('Start')
    ...
    def getAvatarDetails(self, avId):
        ...
        def handleToon(dclass, fields):
            ...
            self.sendUpdateToAvatarId(senderId, 'friendDetails', [avId,
             inventory,
             trackAccess,
             trophies,
             hp,
    ...
    def getPetDetails(self, petId):
        if simbase.config.GetBool('want-pets', True):
            ...
            def handlePet(dclass, fields):
                ...
                self.sendUpdateToAvatarId(senderId, 'petDetails', [petId,
                 fields.get('setOwnerId', [0])[0],
    ...
    def giveUpTeleportQuery(self, fromId, toId):
        if fromId in self.tpRequests:
            ...
            self.sendUpdateToAvatarId(fromId, 'setTeleportResponse', [toId,
             0,
             0,
             0,
             0])
    ...
    def teleportResponse(self, toId, available, shardId, hoodId, zoneId):
        ...
        if self.tpRequests.get(toId) != fromId:
            ...
            return
        self.sendUpdateToAvatarId(toId, 'setTeleportResponse', [fromId,
         available,
         shardId,
         hoodId,
         zoneId])
