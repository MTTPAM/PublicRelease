...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    CogdoRatio = 0
    SuitHoodInfo = [[ToontownGlobals.ToontownCentralOld,
      8,
      20,
      0,
      99,
    ...
    def createNewSuit(self, blockNumbers, streetPoints, toonBlockTakeover = None, cogdoTakeover = None, minPathLen = None, maxPathLen = None, buildingHeight = None, suitLevel = None, suitType = None, suitTrack = None, suitName = None, skelecog = None, revives = None, waiter = None):
        self.skeleChance = 0
        self.isElite = False
        startPoint = None
        blockNumber = None
    ...
    def pathCollision(self, path, elapsedTime):
        ...
        while point.getPointType() == DNASuitPoint.FRONT_DOOR_POINT or point.getPointType() == DNASuitPoint.SIDE_DOOR_POINT:
            i += 1
            lastPi = pi
            pi = path.getPointIndex(i)
            adjacentPoint = point
    ...
    def assignInitialSuitBuildings(self):
        ...
        self.notify.debug('Want %s out of %s total suit buildings; we currently have %s assigned, %s actual.' % (wantedSuitBuildings,
         totalBuildings,
         targetSuitBuildings,
         actualSuitBuildings))
