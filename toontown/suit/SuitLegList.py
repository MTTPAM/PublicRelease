...
class SuitLeg:
    ...
    def getLegTime(self):
        ...
        if self.type in (SuitLeg.TToToonBuilding, SuitLeg.TToCogHQ, SuitLeg.TFromCogHQ):
...
class SuitLegList:
    ...
    def getLegType(self, pointTypeA, pointTypeB):
        if pointTypeA in (DNASuitPoint.FRONT_DOOR_POINT, DNASuitPoint.SIDE_DOOR_POINT):
            return SuitLeg.TWalkToStreet
        if pointTypeB in (DNASuitPoint.FRONT_DOOR_POINT, DNASuitPoint.SIDE_DOOR_POINT):
