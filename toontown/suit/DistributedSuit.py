#Embedded file name: toontown.suit.DistributedSuit
import copy
import math
import random
...
class DistributedSuit(DistributedSuitBase.DistributedSuitBase, DelayDeletable):
    ...
    def __init__(self, cr):
        ...
        self.buildingSuit = 0
        self.fsm = ClassicFSM.ClassicFSM('DistributedSuit', [State.State('Off', self.enterOff, self.exitOff, ['FromSky',
          'FromSuitBuilding',
          'Walk',
          'Battle',
          'neutral',
          ...
          'WalkToStreet',
          'WalkFromStreet']),
         State.State('FromSky', self.enterFromSky, self.exitFromSky, ['Walk',
          'Battle',
          'neutral',
          'ToSky',
          'WalkFromStreet']),
         State.State('FromSuitBuilding', self.enterFromSuitBuilding, self.exitFromSuitBuilding, ['WalkToStreet',
          'Walk',
          'Battle',
          'neutral',
          'ToSky']),
         State.State('WalkToStreet', self.enterWalkToStreet, self.exitWalkToStreet, ['Walk',
          'Battle',
          'neutral',
          'ToSky',
          'ToToonBuilding',
          ...
          'ToCogHQ',
          'WalkFromStreet']),
         State.State('WalkFromStreet', self.enterWalkFromStreet, self.exitWalkFromStreet, ['ToToonBuilding',
          'ToSuitBuilding',
          'ToCogHQ',
          'Battle',
          'neutral',
          'ToSky']),
         State.State('Walk', self.enterWalk, self.exitWalk, ['WaitForBattle',
          'Battle',
          'neutral',
          'WalkFromStreet',
          'ToSky',
         ...
         State.State('WaitForBattle', self.enterWaitForBattle, self.exitWaitForBattle, ['Battle',
          ...
          'ToSuitBuilding',
          'ToSky']),
         State.State('ToToonBuilding', self.enterToToonBuilding, self.exitToToonBuilding, ['neutral', 'Battle']),
         State.State('ToSuitBuilding', self.enterToSuitBuilding, self.exitToSuitBuilding, ['neutral', 'Battle']),
         State.State('ToCogHQ', self.enterToCogHQ, self.exitToCogHQ, ['neutral', 'Battle']),
         State.State('FromCogHQ', self.enterFromCogHQ, self.exitFromCogHQ, ['neutral', 'Battle', 'Walk']),
    ...
    def enterWalkFromStreet(self, leg, time):
        ...
        self.enableRaycast(1)
        h = self.calculateHeading(a, b)
        self.hprScaleInterval(0.3, Vec3(h, 0, 0), Vec3(1, 1, 1)).start()
    ...
    def enterWalk(self, leg, time):
        ...
        self.posQuatScaleInterval(0.3, Point3(fullpos), Vec3(h, 0, 0), Vec3(1, 1, 1)).start()
    ...
    def playCurrentDialogue(self, dialogue, chatFlags, interrupt = 1):
        ...
        if dialogue:
            ...
            if hasattr(base.cr, 'chatLog'):
                base.cr.chatLog.addToLog('\x01cogGray\x01%s\x02: %s' % (self.name, self.nametag.getChatText()))
