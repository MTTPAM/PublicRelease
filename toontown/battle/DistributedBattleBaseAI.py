#Embedded file name: toontown.battle.DistributedBattleBaseAI
import random
...
class DistributedBattleBaseAI(DistributedObjectAI.DistributedObjectAI, BattleBase):
    ...
    def rewardDone(self):
        ...
        if self.ignoreResponses == 1:
            ...
            return
        if stateName not in ('Reward', 'BuildingReward', 'FactoryReward', 'MintReward', 'StageReward', 'CountryClubReward', 'BoardOfficeReward'):
            ...
            if toonId not in self.pets:
                ...
                def handleGetPetProxy(success, pet, petId = petId, zoneId = zoneId, toonId = toonId):
                    if success:
                        ...
                        self.acceptOnce(self.air.getAvatarExitEvent(petId), lambda : taskMgr.doMethodLater(0, deleted, self.uniqueName('petdel-%d' % petId)))
                    else:
