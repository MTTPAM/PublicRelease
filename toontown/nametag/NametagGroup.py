#Embedded file name: toontown.nametag.NametagGroup
...
class NametagGroup:
    ...
    def tick(self, task):
        ...
        if NametagGlobals.forceOnscreenChat and chatText and self.chatBalloonType == NametagGlobals.CHAT_BALLOON:
            ...
            if chatText and self.chatBalloonType == NametagGlobals.CHAT_BALLOON and not base.cam.node().isInView(self.avatar.getPos(base.cam)):
                visible3d = False
            else:
                visible3d = True
        elif NametagGlobals.force2dNametags:
            visible3d = False
        elif not NametagGlobals.want2dNametags and (not chatText or self.chatBalloonType != NametagGlobals.CHAT_BALLOON):
            visible3d = True
    ...
    def setChatText(self, chatText, timeout = False):
        if self.getChatText():
            ...
            self.stompTask = taskMgr.doMethodLater(self.CHAT_STOMP_DELAY, self.__chatStomp, self.stompTaskName, extraArgs=[timeout])
            return
        self.clearChatText()
        ...
        if timeout:
            ...
            self.chatTimeoutTask = taskMgr.doMethodLater(delay, self.clearChatText, self.chatTimeoutTaskName)
