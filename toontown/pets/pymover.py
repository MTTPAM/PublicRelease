#Embedded file name: toontown.pets.pymover
...
class pymover(FSM):
    ...
    def walkToPoint(self, target):
        try:
            ...
            self.__seq = Sequence(Func(self.pet.lookAt, target), Func(self.pet.setP, 0), self.pet.posInterval(dist / self.fwdSpeed, target), Func(self.__stateComplete)).start()
        except:
            return
