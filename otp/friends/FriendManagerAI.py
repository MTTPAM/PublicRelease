...
class FriendManagerAI(DistributedObjectAI):
    ...
    def requestTrueFriendCode(self):
        ...
        if hasattr(self, 'data'):
            ...
            self.air.dbGlobalCursor.trueFriendCodes.insert({'_id': code,
    ...
    def useTrueFriendCode(self, code):
        ...
        if avId == targetId:
            ...
            return
        if av.isTrueFriends(targetId):
            self.sendUpdateToAvatarId(avId, 'trueFriendResponse', [OTPGlobals.TF_ALREADY_FRIENDS, ''])
            return
        if targetId not in av.getFriendsList() and len(av.getFriendsList()) >= OTPGlobals.MaxFriends:
    ...
    def friendQuery(self, requested):
        ...
        self.sendUpdateToAvatarId(requested, 'inviteeFriendQuery', [avId,
         self.air.doId2do[avId].getName(),
         self.air.doId2do[avId].getDNAString(),
         context])
...
class AddTrueFriend:
    ...
    def __gotAvatar(self, dclass, fields):
        ...
        self.air.dbInterface.updateObject(self.air.dbId, self.targetId, self.air.dclassesByName['DistributedToonAI'], {'setFriendsList': [friendsList],
