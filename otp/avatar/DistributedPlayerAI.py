...
class DistributedPlayerAI(DistributedAvatarAI.DistributedAvatarAI, PlayerBase.PlayerBase, ClsendTracker):
    ...
    def d_setTalk(self, fromAV, fromAC, avatarName, chat, mods, flags):
        self.sendUpdate('setTalk', [fromAV,
         fromAC,
         avatarName,
         chat,
         mods,
...
def system(message):
    ...
    dg = dclass.aiFormatUpdate('systemMessage', OtpDoGlobals.OTP_DO_ID_CLIENT_SERVICES_MANAGER, 10, 1000000, [message])
    simbase.air.send(dg)
...
def accessLevel(accessLevel, storage = 'PERSISTENT', showGM = 1):
    accessName2Id = {'user': CATEGORY_USER.defaultAccess,
