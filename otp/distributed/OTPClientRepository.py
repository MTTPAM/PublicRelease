...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher = None, playGame = None):
        ...
        self.accountOldAuth = config.GetBool('%s-account-old-auth' % process, self.accountOldAuth)
        ...
        self.loginFSM = ClassicFSM('loginFSM', [State('loginOff', self.enterLoginOff, self.exitLoginOff, ['connect']),
         State('connect', self.enterConnect, self.exitConnect, ['noConnection',
          'login',
          'failedToConnect',
    ...
    def startLeakDetector(self):
        if hasattr(self, 'leakDetector'):
            return False
        firstCheckDelay = config.GetFloat('leak-detector-first-check-delay', 120.0)
 ...
 'backArgs': []}):
    ...
    def isFreeTimeExpired(self):
        if self.accountOldAuth:
            return 0
        elif base.config.GetBool('free-time-expired', 0):
            return 1
        elif base.config.GetBool('unlimited-free-time', 0):
            return 0
        elif self.freeTimeExpiresAt == -1:
            return 0
        elif self.freeTimeExpiresAt == 0:
            return 1
    ...
    def __playBackGenerates(self, handle):
        ...
        generates.sort(key=lambda i: i[3])
