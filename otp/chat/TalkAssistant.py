...
class TalkAssistant(DirectObject.DirectObject):
            ...
            if not isSpam:
                ...
                if hasattr(base.cr, 'chatLog'):
                    if base.localAvatar.getAdminAccess() >= 375:
                        avatarName = '\x01WLEnter\x01(%s)\x02%s' % (accountId, avatarName)
                    if self.isThought(message):
                        base.cr.chatLog.addToLog('\x01playerGreen\x01%s thinks\x02: %s' % (avatarName, message), senderAvId)
                    else:
                        base.cr.chatLog.addToLog('\x01playerGreen\x01%s\x02: %s' % (avatarName, message), senderAvId)
    ...
    def receiveWhisperTalk(self, avatarId, avatarName, accountId, accountName, toId, toName, message, scrubbed = 0):
        ...
        if hasattr(base.cr, 'chatLog'):
            if base.localAvatar.getAdminAccess() >= 375:
                avatarName = '\x01WLEnter\x01(%s)\x02%s' % (accountId, avatarName)
            base.cr.chatLog.addToLog('\x01playerGreen\x01%s whispers\x02: \x01WLDisplay\x01%s\x02' % (avatarName, message), avatarId)
        return error
    ...
    def receiveThought(self, avatarId, avatarName, accountId, accountName, message, scrubbed = 0):
        ...
        if hasattr(base.cr, 'chatLog'):
            if base.localAvatar.getAdminAccess() >= 375:
                avatarName = '\x01WLEnter\x01(%s)\x02%s' % (accountId, avatarName)
            base.cr.chatLog.addToLog('\x01playerGreen\x01%s thinks\x02: %s' % (avatarName, message), avatarId)
        return error
    ...
    def receiveGameMessage(self, message):
        ...
        messenger.send('NewOpenMessage', [newMessage])
        if hasattr(base.cr, 'chatLog'):
            base.cr.chatLog.addToLog('\x01playerGreen\x01System Message\x02: %s' % message)
        return error
    ...
    def receiveOpenSpeedChat(self, type, messageIndex, senderAvId, name = None):
        ...
        if hasattr(base.cr, 'chatLog'):
            if base.localAvatar.getAdminAccess() >= 375:
                name = '\x01WLEnter\x01(%s)\x02%s' % (senderAvId, name)
            base.cr.chatLog.addToLog('\x01playerGreen\x01%s\x02: %s' % (name, message), senderAvId)
        return error
    ...
    def receiveAvatarWhisperSpeedChat(self, type, messageIndex, senderAvId, name = None):
        ...
        if hasattr(base.cr, 'chatLog'):
            if base.localAvatar.getAdminAccess() >= 375:
                name = '\x01WLEnter\x01(%s)\x02%s' % (senderAvId, name)
            base.cr.chatLog.addToLog('\x01playerGreen\x01%s whispers\x02: \x01WLDisplay\x01%s\x02' % (name, message), senderAvId)
        return error
